...
**当前实现**：Python 解析器对特殊前缀帧**未做值缩放**，直接返回原始值。这意味着如果用户设备发送的是 `HSTDC:` 前缀，m1600 显示的 mT 值会比 DataReader2 **大 10 倍**；`UHSDC:` 则大 **10000 倍**。

**决策**：由于特殊前缀设备极为罕见（可能是早期批次或定制型号），且没有硬件可验证，当前版本保持“原始值直通”，但在日志中记录前缀类型，方便后续校准。
...
**当前实现**：Python 用 `dg2[1]` 取 Y 通道。如果实际硬件发 2 段，我们是正确的；如果发 3 段，需要调整。
...
**当前实现**：m1600 的阈值判断基于 `field_mt`（即 `field_total_mt`），和 DataReader2 行为不同。这是有意为之：我们认为用户更关心总磁场是否超标。如需改为 X 轴-only，可在配置中添加选项。
...
| 3 | A/m | ×795.77 | 标准帧用 795.77，但前缀帧用 79.577 |
| 4 | mGs | ×10000 | |

**注意**：A/m 的系数在标准帧（`795.77`）和前缀帧（`79.577`）中不同，差了 10 倍。这说明不同探头的原始单位可能不同。

### 9.8 审查方法论：如何高效全面审查逆向源码
...
> 最后更新：2026-05-17 | 审查范围：Form1.cs OD_analyse / TD_analyse / TTD_analyse / F_analyse / FG_analyse / TFG_analyse
